...
The printed trail lists each event's timestamp, type, the person and email snapshotted with it, the IP address, and the browser and operating system the event came from — `Chrome 141, macOS` rather than the `User-Agent` header those were read out of (`Taily\Support\UserAgentSummary`). The header itself stays in the database untouched, because it is the evidence; what the certificate prints is a reading of it, short enough for a six-column table on A4 and carrying less of a fingerprint into a document that goes to the adopter and is kept for years. Events no browser triggered — the process being finalized, a link expiring, an email going out — print a dash in both the IP and browser columns, which the table's own introduction says.
