...
1. **Generate.** The mediator picks which contract template to use (Taily supports several at once — see [ADR-013](../ADRs/ADR-013-contract-template-rendering.md#decision) — e.g. one per animal type, or a variant for a specific situation; there's no auto-suggestion in this first version, the mediator always chooses explicitly) and triggers generation from the adoption. Taily renders that template with the animal's, adopter's, mediator's, and organization's data filled in, producing an unsigned PDF. This PDF is rendered exactly once and is not re-rendered later in the flow — see [Document integrity](#document-integrity) for why.
...
5. **Completion.** Once the adopter submits, Taily assembles the final artifact: the original PDF, both signature/audit records, and a human-readable certificate of completion (see [Audit trail](#audit-trail)), stored as the adoption's signed contract. All participants (mediator and adopter) are notified by email. The contract step is marked done — same end state as the Simple Version reaching "signed", just reached automatically.
...
The raw log is the source of truth, but it isn't the artifact a mediator or adopter should have to read to understand what happened — a human-readable "certificate of completion" (both signers, their timestamps, origin details, tied to the final document's hash) is generated alongside the final signed document once both signatures are in, matching the pattern most established e-signature tools (DocuSign, Yousign, DocuSeal) already use. This should be visible to Taily admins, and is also sent to the adopter — they reasonably expect a copy of what they legally committed to, not just the mediator.
