...
- **Features are opt-in.** `resetPasswords`, `updatePasswords`, and `twoFactorAuthentication` are enabled. Registration stays disabled — onboarding remains invitation-based.
- **Two-factor authentication (TOTP) is enabled.** The feature is turned on with `confirm => true`, so enabling generates the secret and recovery codes but the second factor only becomes active once the user verifies a code — a mistyped authenticator secret can never lock anyone out. The management endpoints (enable/confirm/disable, QR code, secret key, recovery codes) and the `/internal/two-factor-challenge` login step are registered explicitly in `routes/internal.php` against Fortify's own controllers, matching the routes-stay-explicit approach above. `ProfileController@show` exposes a `two_factor_enabled` flag (confirmed only) so the SPA can render the current state, and `two_factor_secret` / `two_factor_recovery_codes` are added to the User model's `$hidden` (Fortify's trait does not hide them, and the profile is serialized via `toArray()`). Passkeys (`Features::passkeys()`, available because Fortify depends on `laravel/passkeys`) are deliberately deferred: they add a WebAuthn ceremony layer to both login and settings that warrants its own focused change, and are tracked as follow-up work rather than bundled here.
- **Sensitive 2FA operations require re-authentication.** Disabling 2FA, revealing the secret, and viewing or regenerating recovery codes all expose or weaken the second factor, so every 2FA management route sits behind the `password.confirm` middleware — a stolen or unattended session must re-prove the password (valid for `auth.password_timeout`) before acting. Enrolment shares the same window, so the user is prompted once. This mirrors Fortify's own `confirmPassword` default; because Taily registers these routes itself, the guard is applied as explicit middleware rather than through the feature option (which only affects Fortify-registered routes). The re-authentication endpoints — `POST /internal/user/confirm-password` and `GET /internal/user/confirmed-password-status` (Fortify's `ConfirmablePasswordController` / `ConfirmedPasswordStatusController`) — are registered alongside them, and the SPA checks the status before prompting. The login two-factor challenge itself is not password-gated (the user is mid-login), but still offers a one-time recovery code as an alternative to the TOTP.
...
- Password reset and two-factor authentication arrived essentially for free (broker orchestration, token lifecycle, throttling for reset; TOTP secret/QR/recovery-code handling and the login challenge for 2FA) instead of being hand-written, and passkeys can be adopted on the same foundation without a new dependency.
