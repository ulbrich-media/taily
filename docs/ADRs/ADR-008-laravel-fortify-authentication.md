...
- **Two-factor authentication (TOTP) is enabled.** The feature is turned on with `confirm => true`, so enabling generates the secret and recovery codes but the second factor only becomes active once the user verifies a code — a mistyped authenticator secret can never lock anyone out. `confirmPassword` is left off: the management endpoints already sit behind the authenticated SPA session, and Taily has no separate password-confirmation flow to reuse. The management endpoints (enable/confirm/disable, QR code, secret key, recovery codes) and the `/internal/two-factor-challenge` login step are registered explicitly in `routes/internal.php` against Fortify's own controllers, matching the routes-stay-explicit approach above. `ProfileController@show` exposes a `two_factor_enabled` flag (confirmed only) so the SPA can render the current state. Passkeys (`Features::passkeys()`, available because Fortify depends on `laravel/passkeys`) are deliberately deferred: they add a WebAuthn ceremony layer to both login and settings that warrants its own focused change, and are tracked as follow-up work rather than bundled here.
